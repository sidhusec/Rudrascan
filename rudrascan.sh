...
# SC2154: Variables like bred, reset, bgreen, yellow are defined in rudrascan.cfg
...
# Welcome to RudraScan main script
#██████╗ ██╗   ██╗██████╗ ██████╗  █████╗ ███████╗ ██████╗ █████╗ ███╗   ██╗
#██╔══██╗██║   ██║██╔══██╗██╔══██╗██╔══██╗██╔════╝██╔════╝██╔══██╗████╗  ██║
#██████╔╝██║   ██║██║  ██║██████╔╝███████║███████╗██║     ███████║██╔██╗ ██║
#██╔══██╗██║   ██║██║  ██║██╔══██╗██╔══██║╚════██║██║     ██╔══██║██║╚██╗██║
...
#╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═══╝
 ...
 #       ⚡ RudraScan - Automated Recon & Vulnerability Scanner ⚡
...
. "${SCRIPTPATH}"/rudrascan.cfg || {
    _print_error "Error importing rudrascan.cfg"
    exit 1
}
...
if [[ -s $CUSTOM_CONFIG ]]; then
    # shellcheck source=/home/sidhusec/Tools/rudrascan/custom_config.cfg
    . "${CUSTOM_CONFIG}" || {
...
case $opt_mode in
    ...
    'w')
        if [[ -n $list ]]; then
            ...
            _print_error "Web mode needs a website list file as target (./rudrascan.sh -l target.txt -w)"
            exit $E_INVALID_INPUT
        fi
        webs_menu
        exit 0
