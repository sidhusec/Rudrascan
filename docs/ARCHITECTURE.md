# RudraScan Architecture

## Overview

RudraScan is a modular reconnaissance framework. The codebase is organized as follows:

```
rudrascan/
├── rudrascan.sh          # Main entry point, argument parsing, orchestration
├── rudrascan.cfg         # Main configuration file (all settings)
...
Modules are sourced in this specific order (defined in `rudrascan.sh`):
...
- `$SCRIPTPATH` - Path to rudrascan installation
- `$domain` - Target domain
- `$dir` - Output directory for current scan

### Configuration (from rudrascan.cfg)
