#!/bin/bash
# shellcheck disable=SC2154  # Variables defined in rudrascan.cfg
# RudraScan - Mode/workflow orchestration module
...
# This file is sourced by rudrascan.sh - do not execute directly
[[ -z "${SCRIPTPATH:-}" ]] && {
    echo "Error: This module must be sourced by rudrascan.sh" >&2
    exit 1
}
...
function start() {
    ...
    {
        printf "[%s] RudraScan version: %s\n" "$(date +'%Y-%m-%d %H:%M:%S')" "$rudrascan_version"
...
function end() {
    ...
    if [[ $opt_ai ]]; then
        ...
        local -a ai_cmd
        ai_script="${tools}/rudrascan_ai/rudrascan_ai.py"
        ai_prompts_file="${AI_PROMPTS_FILE:-${tools}/rudrascan_ai/prompts.json}"
        ai_venv_python="${tools}/rudrascan_ai/venv/bin/python3"
        ai_python="${AI_EXECUTABLE:-python3}"
        ai_json_output="${dir}/ai_result/rudrascan_analysis.json"
        ...
        if [[ -n "${ai_python}" && -f "${ai_script}" ]]; then
            ...
            notification "AI skipped: rudrascan_ai script not found at ${ai_script}" warn
        fi
    fi
...
function help() {
    ...
    printf " ./rudrascan.sh -d example.com -r\n"
    printf " \n"
    printf " %bPerform subdomain enumeration on multiple targets:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -l targets.txt -s\n"
    printf " \n"
    printf " %bPerform Web based scanning on a subdomains list:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -d example.com -l targets.txt -w\n"
    printf " \n"
    printf " %bMultidomain recon:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -m company -l domainlist.txt -r\n"
    printf " \n"
    printf " %bPerform full recon (with active attacks) along Out-Of-Scope subdomains list:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -d example.com -x out.txt -a\n"
    printf " \n"
    printf " %bAnalyze RudraScan results with AI:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -d example.com -r --ai\n"
    printf " \n"
    printf " %bRun recon on Axiom with 30 instances:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -d example.com -r -v 30\n"
    printf " \n"
    printf " %bRun custom function:%b\n" "${byellow}" "${reset}"
    printf " ./rudrascan.sh -d example.com -c nuclei_check \n"
}
