#!/bin/bash
# RudraScan - Core framework module
...
function tools_installed() {
    ...
    declare -A tools_files=(
        ...
        ["rudrascan_ai"]="${tools}/rudrascan_ai/rudrascan_ai.py"
        ["rudrascan_ai_python"]="${tools}/rudrascan_ai/venv/bin/python3"
...
function sendToNotify {
    if [[ -z $1 ]]; then
        ...
        if [[ -n $slack_channel ]] && [[ -n $slack_auth ]]; then
            ...
            run_command curl -F file=@${1} -F "initial_comment=rudrascan zip file" -F channels=${slack_channel} -H "Authorization: Bearer ${slack_auth}" https://slack.com/api/files.upload 2>>"$LOGFILE" >/dev/null
        fi
    fi
}
...
function plugins_emit() {
    local event="$1"
    shift || true
    if declare -F rudrascan_plugins >/dev/null 2>&1; then
        rudrascan_plugins "$event" "$@" || true
    fi
}
...
<head>
  ...
  <title>RudraScan Report - ${domain}</title>
  <style>
...
<body>
  <div class="wrap">
    <h1>RudraScan Consolidated Report</h1>
