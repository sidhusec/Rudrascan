...
CONFIG_FILE="./rudrascan.cfg"

if [[ ! -f $CONFIG_FILE ]]; then
    printf "%b[!] Config file rudrascan.cfg not found.%b\n" "$bred" "$reset"
    exit 1
fi

# shellcheck source=./rudrascan.cfg
...
declare -A repos=(
    ...
    ["rudrascan_ai"]="six2dez/rudrascan_ai"
...
╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═══╝
 ...
 ${rudrascan_version}                                         by @sidhusec ,@sanjay_harsha,@rajini_k,@sai_charan
...
function check_updates() {
    printf "%bRunning: Looking for new RudraScan version%b\n" "$bblue" "$reset"
    ...
    if { [[ -n $TIMEOUT_CMD ]] && $TIMEOUT_CMD 10 git fetch; } || git fetch; then
        ...
        if [[ $HEADHASH != "$UPSTREAMHASH" ]]; then
            ...
            if git status --porcelain | grep -q 'rudrascan.cfg$'; then
                cfg_backup_ts=$(date +%Y%m%d_%H%M%S)
                cfg_backup="rudrascan.cfg.bak.${cfg_backup_ts}"
                cp rudrascan.cfg "$cfg_backup"
                printf "%brudrascan.cfg has been backed up to %s%b\n" "$yellow" "$cfg_backup" "$reset"
            fi
            ...
            if [[ -n $cfg_backup && -f rudrascan.cfg ]]; then
                mkdir -p .tmp
                cfg_diff_file=".tmp/rudrascan_cfg_diff_${cfg_backup_ts}.patch"
                if diff -u "$cfg_backup" rudrascan.cfg >"$cfg_diff_file"; then
            ...
            printf "%bRudraScan is already up to date!%b\n" "$bgreen" "$reset"
            # If config is locally modified, still provide a diff against current default
            if ! git diff --quiet -- rudrascan.cfg; then
                local cfg_diff_ts cfg_diff_file
                cfg_diff_ts=$(date +%Y%m%d_%H%M%S)
                mkdir -p .tmp
                cfg_diff_file=".tmp/rudrascan_cfg_diff_${cfg_diff_ts}.patch"
                if git diff HEAD --unified -- rudrascan.cfg >"$cfg_diff_file"; then
                    printf "%bLocal rudrascan.cfg differs from default; diff saved to %s.%b\n" "$yellow" "$cfg_diff_file" "$reset"
                fi
            fi
        fi
    else
    ...
    if [[ $install_golang == "true" ]]; then
        ...
        if [[ -n $current_version && $version == "$current_version" ]]; then
            ...
            if [[ -d /usr/local/go ]]; then
                go_backup="/usr/local/go.rudrascan.$(date +%s)"
        ...
        if [[ -n ${profile_shell:-} ]]; then
            ...
            local marker="# Golang environment variables (RudraScan)"

            # Remove ALL previous golang env blocks (old-style and RudraScan-style)
        ...
        msg_warn "Golang will not be configured according to the user's preferences (install_golang=false in rudrascan.cfg)."
    fi

    if ! command -v go &>/dev/null; then
        msg_err "[!] Go binary not found in PATH. Please install Go or enable install_golang in rudrascan.cfg."
        return 1
    fi
...
# Setup rudrascan venv for python-only helpers (e.g., getjswords)
function setup_rudrascan_venv() {
    local root_dir="${SCRIPTPATH:-$(pwd)}"
    local venv_dir="${root_dir}/.venv"
    if ! command -v python3 >/dev/null 2>&1; then
        msg_warn "[!] python3 not found; skipping rudrascan venv setup"
        return 0
    fi
    if [[ ! -d "$venv_dir" ]]; then
...
- SSRF Server (COLLAB_SERVER in rudrascan.cfg or env var)
- Blind XSS Server (XSS_SERVER in rudrascan.cfg or env var)
- notify (${HOME}/.config/notify/provider-config.yaml)
- WHOISXML API (WHOISXML_API in rudrascan.cfg or env var)
EOF
    )"
...
function handle_install_arguments() {
    printf "\n%bRudraScan installer/updater script%b\n" "$bgreen" "$reset"
    ...
    if [[ "$(id -u)" -eq 0 ]]; then
        ...
        if ! sudo -n true 2>/dev/null; then
            ...
            printf "%becho \"%s  ALL=(ALL:ALL) NOPASSWD: ALL\" | sudo tee /etc/sudoers.d/RudraScan%b\n\n" "$bred" "$USER" "$reset"
        fi
        SUDO="sudo"
    fi
}
