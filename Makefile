...
PRIVATE_REPO := $(shell echo $${PRIV_REPO-rudrascan-data})
...
help:
	@echo "RudraScan Development Commands"
	@echo ""
...
lint:
	@if command -v shellcheck >/dev/null 2>&1; then \
		shellcheck -S error rudrascan.sh modules/*.sh lib/*.sh install.sh; \
	else \
...
lint-fix:
	@if command -v shellcheck >/dev/null 2>&1; then \
		shellcheck -S warning -f gcc rudrascan.sh modules/*.sh lib/*.sh install.sh; \
	else \
		echo "shellcheck not found. Install: https://www.shellcheck.net/"; \
		exit 1; \
	fi
...
fmt:
	@if command -v shfmt >/dev/null 2>&1; then \
		shfmt -w -i 4 -bn -ci install.sh rudrascan.sh modules/*.sh lib/*.sh; \
	else \
...
test-release-gate:
	@bash -n rudrascan.sh modules/*.sh lib/*.sh
	@./tests/run_tests.sh --unit
	@./tests/run_tests.sh --smoke
