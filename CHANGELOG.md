...
**Config Profiles**
- Full profile (`rudrascan_full.cfg`): enables `PTR_SWEEP`, `SRV_ENUM`, `NS_DELEGATION`, `TLS_IP_PIVOTS`, `JS_SUB_EXTRACT`, `WELLKNOWN_PIVOTS`.
- Quick profile (`rudrascan_quick.cfg`): enables only `SRV_ENUM` (lightweight); disables all others.
- Stealth profile (`rudrascan_stealth.cfg`): enables only `SRV_ENUM`; disables all active techniques.
...
- AI report generation integrated with local models via `rudrascan_ai`.
- AI profiles: `executive`, `brief`, `bughunter`.
- Structured AI output artifact: `ai_result/rudrascan_analysis.json`.
- Human AI report artifact: `ai_result/rudrascan_analysis_<profile>_<timestamp>.md|txt`.
...
- New profile configs:
  - `config/rudrascan_quick.cfg`
  - `config/rudrascan_full.cfg`
  - `config/rudrascan_stealth.cfg`
...
- Added configurable parallel UX and sizing knobs in `rudrascan.cfg` (`PARALLEL_UI_MODE`, progress visibility toggles, and per-group parallel sizes).
