...
# Unit tests for RudraScan utility functions
...
setup() {
    ...
    source "$project_root/rudrascan.cfg" 2>/dev/null || true
    export SCRIPTPATH="$project_root"
    source "$project_root/rudrascan.sh" --source-only
}
...
@test "check_secrets_permissions warns on world-readable file" {
    ...
    run bash -c "
        source '$SCRIPTPATH/rudrascan.sh' --source-only
