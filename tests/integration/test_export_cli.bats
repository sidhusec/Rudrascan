...
@test "export cli json creates jsonl artifacts" {
  run timeout 30 bash "$SCRIPTPATH/rudrascan.sh" -d "$TEST_DOMAIN" --report-only --export json
  [ "$status" -eq 0 ]
  [ -f "$TARGET_DIR/report/findings_normalized.jsonl" ]
}

@test "export cli csv creates csv artifacts" {
  run timeout 30 bash "$SCRIPTPATH/rudrascan.sh" -d "$TEST_DOMAIN" --report-only --export csv
  [ "$status" -eq 0 ]
