...
@test "help advertises --vps-count" {
  run timeout 20 bash "$SCRIPTPATH/rudrascan.sh" -h
  [ "$status" -eq 0 ]
  [[ "$output" == *"--vps-count n"* ]]
}

@test "--vps-count rejects 0" {
  run timeout 20 bash "$SCRIPTPATH/rudrascan.sh" --vps-count 0 -h 2>&1
  [ "$status" -ne 0 ]
  [[ "$output" == *"Invalid --vps-count value"* ]]
}

@test "--vps-count rejects non-numeric values" {
  run timeout 20 bash "$SCRIPTPATH/rudrascan.sh" --vps-count abc -h 2>&1
  [ "$status" -ne 0 ]
  [[ "$output" == *"Invalid --vps-count value"* ]]
}

@test "--vps-count accepts valid value" {
  run timeout 20 bash "$SCRIPTPATH/rudrascan.sh" --vps-count 30 -h
  [ "$status" -eq 0 ]
}

@test "--report-only accepts --vps-count override parsing" {
  run env SKIP_CRITICAL_CHECK=true timeout 30 bash "$SCRIPTPATH/rudrascan.sh" \
...
@test "--vps-count override forces fleet launch/count over custom config values" {
  run env PATH="$MOCK_BIN:$PATH" SKIP_CRITICAL_CHECK=true timeout 30 bash "$SCRIPTPATH/rudrascan.sh" \
