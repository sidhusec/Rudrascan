...
@test "--quiet hides header/sections but keeps final summary" {
    run timeout 45 bash "$SCRIPTPATH/rudrascan.sh" -d example.com -s --dry-run --no-report --quiet 2>&1
    [ "$status" -eq 0 ]
...
@test "--log-format jsonl-strict emits JSONL lines only" {
    run timeout 45 bash "$SCRIPTPATH/rudrascan.sh" -d example.com -s --dry-run --no-report --log-format jsonl-strict 2>&1
    [ "$status" -eq 0 ]
