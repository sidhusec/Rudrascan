...
# Integration tests for RudraScan full workflow
...
setup() {
    ...
    export TEST_DIR="$BATS_TEST_TMPDIR/rudrascan_test"
    mkdir -p "$TEST_DIR"
...
# Helper to source rudrascan without running main
source_rudrascan() {
    # shellcheck source=/dev/null
    source "$SCRIPTPATH/rudrascan.sh" --source-only 2>/dev/null || true
}
...
@test "full flow: dry-run mode prevents execution" {
    run timeout 5 bash "$SCRIPTPATH/rudrascan.sh" -d test.com --dry-run 2>&1
