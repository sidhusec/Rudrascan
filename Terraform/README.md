# RudraScan loves Ansible+Terraform <3
...
> package repos change over time, so you may need to adjust it for your environment. It is not covered by RudraScan CI.
...
## Deploying RudraScan on the AWS Cloud using Terraform and Ansible
...
1. Move to the Terraform folder (optional but recommended): `cd Terraform`
    - Put your own Subfinder config file and RudraScan config file on the files/ folder
...
1. ???
1. If you have finished playing with RudraScan and hacking the world, remember to destroy the instance using `terraform destroy`
