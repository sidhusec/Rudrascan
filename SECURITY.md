...
RudraScan handles API keys and credentials in three ways (in order of precedence):
...
Create a `secrets.cfg` file in the RudraScan root directory. It is automatically sourced at startup and is listed in `.gitignore`:
...
| `rudrascan.cfg` (with keys) | Configuration with API keys filled in |
...
RudraScan sanitizes all user-supplied input to prevent command injection:
...
These variables in `rudrascan.cfg` control security and operational safety:
