#!/usr/bin/env bash
# lib/parallel.sh - Parallel execution utilities for RudraScan
# Part of RudraScan refactoring - Phase 3
...
parallel_funcs() {
    ...
    if [[ -n "${dir:-}" ]]; then
        ...
        parallel_log_root="$(mktemp -d /tmp/rudrascan_parallel.XXXXXX)"
    fi
