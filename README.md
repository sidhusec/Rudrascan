...
<h1 align="center">
  <br>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://github.com/six2dez/rudrascan/blob/main/images/banner.png" alt="rudrascan"></a>
  <br>
  RudraScan
  <br>
</h1>
...
<p align="center">
  <a href="https://github.com/six2dez/rudrascan/releases/tag/v4.1"><img src="https://img.shields.io/badge/release-v4.1-2ea043?style=for-the-badge" alt="Release"></a>
  <a href="https://opensource.org/licenses/MIT"><img src="https://img.shields.io/badge/License-MIT-yellow.svg?style=for-the-badge" alt="License"></a>
  <a href="https://github.com/six2dez/rudrascan/issues?q=is%3Aissue+is%3Aclosed"><img src="https://img.shields.io/github/issues-closed-raw/six2dez/rudrascan.svg?style=for-the-badge" alt="Closed Issues"></a>
  ...
  <a href="https://t.me/joinchat/H5bAaw3YbzzmI5co"><img src="https://img.shields.io/badge/telegram-@RudraScan-26A5E4?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram"></a>
...
<p align="center">
  <a href="https://docs.rudrascan.com"><img src="https://img.shields.io/badge/GitBook-%23000000.svg?style=for-the-badge&logo=gitbook&logoColor=white" alt="Docs"></a>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://img.shields.io/badge/Bash%20Script-%23121011.svg?style=for-the-badge&logo=gnu-bash&logoColor=white" alt="Bash Script"></a>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Linux"></a>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://img.shields.io/badge/macOS-000000?style=for-the-badge&logo=macos&logoColor=F0F0F0" alt="macOS"></a>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"></a>
</p>

<p align="center">
  <a href="https://github.com/six2dez/rudrascan/actions"><img src="https://img.shields.io/badge/github_actions-%232671E5.svg?style=for-the-badge&logo=githubactions&logoColor=white" alt="GitHub Actions"></a>
  <a href="https://hub.docker.com/r/six2dez/rudrascan"><img src="https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white" alt="Docker"></a>
  <a href="https://github.com/six2dez/rudrascan/tree/main/Terraform"><img src="https://img.shields.io/badge/terraform-%23844FBA.svg?style=for-the-badge&logo=terraform&logoColor=white" alt="Terraform"></a>
  <a href="https://github.com/six2dez/rudrascan/tree/main/Terraform"><img src="https://img.shields.io/badge/ansible-%231A1918.svg?style=for-the-badge&logo=ansible&logoColor=white" alt="Ansible"></a>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://img.shields.io/badge/Go-00ADD8.svg?style=for-the-badge&logo=go&logoColor=white" alt="Go"></a>
  <a href="https://github.com/six2dez/rudrascan"><img src="https://img.shields.io/badge/Python-3776AB.svg?style=for-the-badge&logo=python&logoColor=white" alt="Python"></a>
</p>
...
**RudraScan** is a powerful automated reconnaissance tool designed for security researchers and penetration testers. It streamlines the process of gathering intelligence on a target by performing subdomain enumeration, vulnerability scanning, OSINT and more. With a modular design, extensive configuration options, and support for distributed scanning via AX Framework, RudraScan is built to deliver comprehensive results efficiently.

RudraScan leverages a wide range of techniques, including passive and active subdomain discovery, web vulnerability checks (e.g., XSS, SSRF, SQLi), OSINT, directory fuzzing, port scanning and screenshotting. It integrates with cutting-edge tools and APIs to maximize coverage and accuracy, ensuring you stay ahead in your reconnaissance efforts.
...
**Disclaimer:** Usage of RudraScan for attacking targets without prior consent is illegal. It is the user's responsibility to obey all applicable laws. The developers assume no liability for misuse or damage caused by this tool. Use responsibly.
...
RudraScan is packed with features to make reconnaissance thorough and efficient. Below is a detailed breakdown of its capabilities, updated to reflect the latest functionality in the script and configuration.
...
- **Docker Support**: Official Docker image on [DockerHub](https://hub.docker.com/r/six2dez/rudrascan).
...
- **AI Report Generation**: Generates reports using local AI models ([rudrascan_ai](https://github.com/six2dez/rudrascan_ai)).
...
RudraScan uses a modular architecture. The main entry point (`rudrascan.sh`) handles argument parsing and sources 8 specialized modules from the `modules/` directory.

### Directory Layout

```
rudrascan/
├── rudrascan.sh          # Entry point — arg parsing, module loading, dispatch
├── rudrascan.cfg         # Default configuration
...
The `--source-only` flag allows sourcing `rudrascan.sh` without executing the main logic, enabling unit testing of individual functions.

---

## 💿 Installation

RudraScan supports multiple installation methods to suit different environments. Ensure you have sufficient disk space (at least 10 GB recommended) and a stable internet connection.
...
```bash
./rudrascan.sh -d example.com -r
```

3) Minimal run (passive-only footprint)

```bash
./rudrascan.sh -d example.com -p
```
...
1. **Prerequisites**:
   ...
   - **Golang**: Latest version (`install_golang` enabled by default in `rudrascan.cfg`).
   - **System Permissions**: If not running as root, configure sudo to avoid prompts:
     ```bash
     sudo echo "${USERNAME} ALL=(ALL:ALL) NOPASSWD: ALL" | sudo tee -a /etc/sudoers.d/RudraScan
     ```
...
- Set `install_golang=false` in `rudrascan.cfg` if Golang is already configured.
...
- Check the [Installation Guide](https://github.com/six2dez/rudrascan/wiki/0.-Installation-Guide) for detailed instructions.
...
2. **Run the Container**:
   ...
   ```bash
   docker run -it --rm \
     -v "${PWD}/OutputFolder/:/rudrascan/Recon/" \
     six2dez/rudrascan:main -d example.com -r
   ```
   ...
   ```bash
   docker run -it --rm \
     -v "${PWD}/domains.txt:/rudrascan/domains.txt:ro" \
     -v "${PWD}/OutputFolder/:/rudrascan/Recon/" \
     six2dez/rudrascan:main -l /rudrascan/domains.txt -r
   ```
...
4. **Customization**:
   - Modify the Docker image or build your own; see the [Docker Guide](https://github.com/six2dez/rudrascan/wiki/4.-Docker).
...
6. **Health Check**:
   ...
   The Docker image includes a built-in `HEALTHCHECK` that runs `./rudrascan.sh --health-check` every 60 seconds. You can also run it manually:

   ```bash
   docker exec <container-id> ./rudrascan.sh --health-check
   ```

### Terraform + Ansible

- Deploy RudraScan on AWS using Terraform and Ansible.
...
- Network hiccups: Installers hide most command output. If something fails, re-run with `upgrade_tools=true` in `rudrascan.cfg`, execute `./install.sh --tools`, or install the missing tool manually (the error will name it).
...
- WHOISXML: set `WHOISXML_API` in `rudrascan.cfg` or env var
...
The `rudrascan.cfg` file controls the entire execution of RudraScan. It allows fine-grained customization of:
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
	rudrascan_version="$(git rev-parse --abbrev-ref HEAD)-$(git describe --tags 2>/dev/null || git rev-parse --short HEAD)"
else
	rudrascan_version="standalone"
fi # Fetch current rudrascan version
...
INSCOPE=false # Uses inscope tool to filter the scope, requires .scope file in rudrascan folder
...
AXIOM_FLEET_NAME="RudraScan" # Fleet's prefix name
...
# This is a script on your rudrascan host that might prep things your way...
...
FARADAY_WORKSPACE="rudrascan" # Faraday workspace

# AI
AI_EXECUTABLE="python3" # Python executable fallback if rudrascan_ai venv python is not available
...
AI_PROMPTS_FILE="" # Optional custom prompts file (empty uses rudrascan_ai default)
...
AI_ALLOW_MODEL_PULL=false # Allow rudrascan_ai to auto-pull missing model
...
**Full Details**: See the [Configuration Guide](https://github.com/six2dez/rudrascan/wiki/3.-Configuration-file).

---

## 🚀 Usage

RudraScan supports multiple modes and options for flexible reconnaissance. Use the `-h` flag to view the help menu.
...
2. **Recon on Multiple Targets**:
   ...
   ```bash
   ./rudrascan.sh -l targets.txt -r -o /path/to/output/
   ```

3. **Deep Recon (VPS Recommended)**:

   ```bash
   ./rudrascan.sh -d target.com -r --deep
   ```

4. **Parallel Mode (Faster, requires more RAM)**:

   ```bash
   ./rudrascan.sh -d target.com -r --parallel
   ```

5. **Force Sequential Mode**:

   ```bash
   ./rudrascan.sh -d target.com -r --no-parallel
   ```
6. **Multi-Domain Recon**:

   ```bash
   ./rudrascan.sh -m company -l domains.txt -r
   ```

7. **Axiom Integration**:

   ```bash
   ./rudrascan.sh -d target.com -r -v
   ```

8. **Axiom Integration with fleet override**:

   ```bash
   ./rudrascan.sh -d target.com -r -v 30
   ```
...
11. **Force cache refresh**:
   ```bash
   ./rudrascan.sh -d target.com -r --refresh-cache
   ```

12. **Export all report artifacts**:
   ```bash
   ./rudrascan.sh -d target.com -r --export all
   ```

13. **Continuous monitoring (every 30m, 48 cycles)**:
   ```bash
   ./rudrascan.sh -d target.com -r --monitor --monitor-interval 30 --monitor-cycles 48
   ```

14. **Rebuild reports only (no scan):**
   ```bash
   ./rudrascan.sh -d target.com --report-only --export all
   ```

**Full Guide**: See the [Usage Guide](https://github.com/six2dez/rudrascan/wiki/2.-Usage-Guide).

---

## ☁️ Ax Framework Support (previously Axiom)

RudraScan integrates with [Ax](https://github.com/attacksurge/ax) for distributed scanning, reducing execution time by distributing tasks across multiple cloud instances.

- **Setup**: Select `rudrascan` as the provisioner during Axiom configuration.
...
- **Configuration**: Set fleet size, region, and name in `rudrascan.cfg` (`AXIOM_FLEET_COUNT`, `AXIOM_FLEET_REGIONS`, `AXIOM_FLEET_NAME`).
...
**Details**: See the [Axiom Guide](https://github.com/six2dez/rudrascan/wiki/5.-Axiom-version) and official [Ax Docs](https://ax.attacksurge.com/).

---

## 💻 Faraday Support

RudraScan integrates with [Faraday](https://faradaysec.com/) for web-based reporting and vulnerability management.

- **Setup**: Install Faraday, authenticate via `faraday-cli`, and configure the workspace in `rudrascan.cfg` (`FARADAY_WORKSPACE`).
- **Usage**: Enable with `FARADAY=true` in `rudrascan.cfg`.

---

## 🧠 AI Integration

RudraScan uses AI to generate detailed reports from scan results with the tool [rudrascan_ai](https://github.com/six2dez/rudrascan_ai).
...
- **Structured Output**: `rudrascan` stores a machine-readable report in `ai_result/rudrascan_analysis.json`.
...
When `ASSET_STORE=true`, RudraScan aggregates key findings into `assets.jsonl` during each run, making it easy to sync only actionable deltas to your private repo.
...
1. **Bootstrap**:
   ...
   ```bash
   export PRIV_REPO="$HOME/rudrascan-data"
   make bootstrap
   ```
...
4. **Lint / Format Scripts**:
   ...
   ```bash
   make lint   # shellcheck for rudrascan.sh, modules/*.sh & install.sh
...
2. Clone and configure:
   ...
   ```bash
   git clone https://github.com/yourusername/rudrascan-data
   cd rudrascan-data
   git commit --allow-empty -m "Initial commit"
   git remote add upstream https://github.com/six2dez/rudrascan
   git fetch upstream
...
RudraScan uses [bats-core](https://github.com/bats-core/bats-core) for automated testing.
...
setup() {
    source ./rudrascan.sh --source-only
}
...
1. **ShellCheck** — lints `rudrascan.sh`, `modules/*.sh`, and `install.sh`
2. **Unit Tests** — runs all `tests/unit/*.bats` files
3. **Integration Tests** — installs RudraScan and validates tool availability
...
- [Report a Bug](https://github.com/six2dez/rudrascan/issues/new/choose)
- [Submit a Pull Request](https://github.com/six2dez/rudrascan/tree/dev) (target the `dev` branch)
...
- **Wiki**: Explore the [RudraScan Wiki](https://github.com/six2dez/rudrascan/wiki).
- **FAQ**: Check the [FAQ](https://github.com/six2dez/rudrascan/wiki/7.-FAQs).
...
Support RudraScan’s development through:
...
Special thanks to the following services for supporting RudraScan:
...
```
rudrascan/
├── rudrascan.sh          # Main entry point (~500 lines)
├── rudrascan.cfg         # Configuration file
...
# 3. Run health check
./rudrascan.sh --health-check

# 4. Dry run to preview
./rudrascan.sh -d example.com -r --dry-run
```
...
RudraScan is licensed under the [MIT License](LICENSE).

---

## ⭐ Star History

[![Star History Chart](https://api.star-history.com/svg?repos=six2dez/rudrascan&type=Date)](https://www.star-history.com/#six2dez/rudrascan&Date)
